...
public class OKW_Ini
{
	...
	* \~
	* \author Zoltán Hrabovszki
	* \date 2014-10-25/jnic
	 */
	public OKW_Enviroment		OKW_Enviroment		= new OKW_Enviroment();

	/*
	* \brief
	* Singelton-Pattern: Feld enthält die einzige gültige Instanz dieser
	* Klasse.
	*
	*
	* \~english* \brief
	...
	* \date 2014-10-25/jnic
	 */
	private static OKW_Ini		Instance;
	...
	* Konstruktor erzeugt,
	* sondern über das Property OKW_Ini.Instance aufgerufen.
	*
	* __Wichtig:__ Verwende nicht diesen Konstuktor, um eine Instanz der
	* Klasse zu erstellen.
	*
	* __Anmerkung:__ Der Konstruktor sollte bei einem Singelton 'private'
	* sein.
	...
	* constructor,
	* but called with the property OKW_Ini.Instance.
	*
	* __Important:__ Do not use this constructor to create an instance of the
	* class.
	*
	* __Note:__ The constructor should be 'private' with a singleton.
	* This constructor must be 'public' because of the serializing!
	*
	* \note Further Information to the topic Singeleton
	* at http://csharpindepth.com/Articles/General/Singleton.aspx
	*
	*
	* \~
	* \author Zoltán Hrabovszki
	* \date 2014-10-25/jnic
	...
	/*
	* \brief
	* Singelton-Pattern: Instanz gibt die aktuelle, gültige und einzige
	...
	* \~english
	* \brief
	* Singelton-Pattern: instance returns the actual valid and only instance
	* of the class.
	*
	* At the first use of this class an instance of this class is
	* automatically created.
	*
	* __Important:__ To receive an instance of OKW.OKW_Ini use exclusively
	* this property only!
	*
	*
	* \~
	...
	* \date 2014-10-25/jnic
	 */
	public static OKW_Ini getInstance()
	{
		Log.LogFunctionStartDebug("OKW_Ini.getInstance");
		if (Instance == null)
		{
			Instance = new OKW_Ini();
			Instance.Init();
		}
	...
	*
	*
	* - Löschen aller Klassen-Variablen der OKW_Ini.
	* - Lesen der Umgebungsvariable OKW_Xml
	* + wenn die Umgebunsvariable vorhanden ist...
	* * und die Datei existiert, dann lesen der Datei.
	* * und mit diesen Werten eine Datei an der gegebene Stelle erstellen.
	* + wenn die Umgebungsvariable fehlt oder nicht gesetzt ist,
	* * dann werden alle werden alle Eigenschaften auf "Default-Wert" gesetzt.
	* * Es wird jedoch keine Datei geschrieben.(Weil die Zieldatei Fehlt!)
	* * Warnung wird ausgegeben, dass die Umgebungsvariable fehlt.
	*
	* \exception OKWEnviromentVarNotSetException Wenn die Umgebungsvariable
	* OKW_Xml nicht gesetzt ist.
	* \exception OKWFileDoesNotExistsException wird ausgelöst, wenn die Datei,
	* gegeben als OKW_Ini.Xml_Ini_xml, nicht
	...
	*
	* @startuml{OKW_Ini_en.png}
	* start
	* :Clear class variables;
	* :read environment variable **OKW_Xml**;
	* if (is **OKW_Xml** defined?) then (yes)
	*
	* if (**OKW_Xml** exists as file?) then (yes)
	* :read data\naus **OKW_Xml**;
	* else (no)
	* :**OKW_xml** create\ntemplatefile;
	...
	* OKW_Xml is not set.
	* \exception OKWFileDoesNotExistsException is triggered, if the file,
	* given as OKW_Ini.Xml_Ini_xml,
	* is not existing. Before a template file was created.
	*
	*
	* \~
	* \author Zoltán Hrabovszki
	* \date 2014-10-25/jnic
	...
	public void Init()
	{
		...
		try
		{
			...
			// Existiert die Datei?
			Boolean bXML_Folder_Exists = OKW.OKW_FileHelper.DirectoryExists(this.OKW_Enviroment.getFolder_XML());

			if (bXML_Folder_Exists)
			{
				this.OKW_Enviroment.setFolder_LogMessages( Paths.get(this.OKW_Enviroment.getFolder_XML(), "LogMessages").toString() );
				this.OKW_Enviroment.setFile_OKW_Ini_xml( Paths.get(this.OKW_Enviroment.getFolder_XML(), "OKW_Ini.xml").toString() );
				// Datei Existiert -> Lesen der Daten

				// Existiert die Datei?
				Boolean bFile_OKW_Ini_xml_Exists = OKW.OKW_FileHelper.FileExists(this.OKW_Enviroment.getFile_OKW_Ini_xml());
	...
	/*
	* \brief
	* Liest die Eigenschaften der Klasse OKW_Ini aus einer Datei, gegeben in
	* OKW.OKW_Ini.Xml_Ini_xml, ein.
	* Es wird eine XML Datei geschrieben. Hierzu wird die Klasse OKW_Ini mit
	* System.Xml.XmlSerializer serialisiert.
	*
	*
	* \~english
	* \brief
	* Reads the properties of the class OKW_Ini from a file, given in
	* OKW.OKW_Ini.Xml_Ini_xml.
	* An XML file is written. For this purpose the class OKW_Ini is serialized
	* with System.Xml.XmlSerializer.
	*
	*
	* \~
	* \author Zoltán Hrabovszki
	* \date 2014-10-25/jnic
	...
	public void Read()
	{
		...
		try
		{
			XmlSerializer serializer = new XmlSerializer(typeof(OKW_Ini));
			StreamReader fs = new StreamReader(this.OKW_Enviroment.File_OKW_Ini_xml);
			_Instance = (OKW_Ini) serializer.Deserialize(fs);
			fs.Close();
		}
		finally
		{
	...
	public void LogAll()
	{
		...
		try
		{
			...
			Log.LogPrint("----------------------------------------------------------------");
			Log.LogPrint("OKW_ImplementationMatrix.xml Path: " + this.OKW_Enviroment.getFile_OKW_ImplementationMatrix_xml());
			Log.LogPrint("----------------------------------------------------------------");
	...
	/*
	* \brief
	* Schreibt die Eigenschaften der Klasse OKW_Ini in eine Datei, gegeben in
	* OKW.OKW_Ini.Xml_Ini_xml.
	* Es wird eine XML Datei geschrieben. Hierzu wird die Klasse OKW_Ini mit
	* der Klasse System.Xml.XmlSerializer serialisiert.
	*
	*
	* \~english
	* \brief
	* Writes the property of the class OKW_Ini in a file, given in
	* OKW.OKW_Ini.Xml_Ini_xml.
	* An XML file is written. For this purpose the class OKW_Ini with the
	* class System.Xml.XmlSerializer is serialized.
	*
	*
	* \~
	* \author Zoltán Hrabovszki
	* \date 2014-10-25/ rev.
	...
	public void Save()
	{
		Log.LogFunctionStartDebug(this.GetType().FullName + ".Save()");

		try
		{
			XmlSerializer serializer = new XmlSerializer(typeof(OKW_Ini));
			StreamWriter fs = new StreamWriter(this.OKW_Enviroment.File_OKW_Ini_xml, false);
	...
	/*
	* \brief
	* Ermittelt den Pfad zu assambly _OKW.dll_.
	*
	*
	* \~english
	* \brief
	* Gets the path to the assembly _OKW.dll_.
	*
	* \~
	* \author Zoltán Hrabovszki
	* \date 2014-10-25/ rev.
