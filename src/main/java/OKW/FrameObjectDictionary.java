...
/// \date 2014.10.10
/// 
public class FrameObjectDictionary
{
	...
	public Map<String, Object>	__FrameObjectDictionary	= new HashMap<String, Object>();
	...
	/// gespeichert.
	///
	/// \see Für das Kindobjekt existiert eine zweite Ausprägung dieser Methode:
	/// FrameObjectDictionary.GetObjectByName(String,String)
	///
	...
	public Object GetParentObjectByName( String fpsParentObject )
	{
		...
		Log.LogFunctionStartDebug("FrameObjectDictionary.GetParentObjectByName", "fpsFunctionalnameOfWindow",
				fpsParentObject);
		try
		{
			...
			if (this.__FrameObjectDictionary.containsKey(fpsParentObject))
			{
				// Ja, das Objekt existiert.
				Log.LogPrintDebug(LM.GetMessage("GetParentObjectByName", "M2"));
				lvo_Return = this.__FrameObjectDictionary.get(fpsParentObject);
			}
			else
			{
				...
				String lvsMessage = LM.GetMessage("GetParentObjectByName", "OKWFrameObjectParentNotFoundException",
						fpsParentObject);
	...
	public Object GetChildObjectByName( String fps_ParentObject, String fps_ChildObject )
	{
		Object lvo_Return = null;
		String lvs_ObjectName = fps_ParentObject + "." + fps_ChildObject;

		Log.LogFunctionStartDebug("FrameObjectDictionary.GetChildObjectByName", "fps_ParentObject", fps_ParentObject,
		...
		if (this.__FrameObjectDictionary.containsKey(lvs_ObjectName))
		{
			// Ja, das Objekt existiert.
			Log.LogPrintDebug(LM.GetMessage("GetChildObjectByName", "M2"));
			// Hole refarenc auf des Objekt im Dictionary
			lvo_Return = this.__FrameObjectDictionary.get(lvs_ObjectName);
		}
		else
		{
			// Nein -> Frame zum Funtionalen Name wurde nicht gefunden ->
			...
			String lvsMessage = LM.GetMessage("GetChildObjectByName", "OKWFrameObjectChildNotFoundException",
					lvs_ObjectName);
	...
	public void Init() throws ClassNotFoundException, InstantiationException
	{
		...
		try
		{
			...
			Log.LogPrint(LM.GetMessage("Init", "InitClear", this.getClass().getName()));
			this.__FrameObjectDictionary.clear();
	...
	{
		String lvsNamespace = "OKW.Frames";
		ArrayList<Class<?>> lvLsReturn = new ArrayList<Class<?>>();

		// 1. Hole Alle namen der Classen, die sich im Package "OKW.Frames"
		// befinden
		ArrayList<Class<?>> lvsClassNames = ClassFinder.getClassesFromPackage(lvsNamespace);

		// 2. Hole alle Classen, die Annotoation "OKWGUI" haben.
		for (Class<?> lvsClass : lvsClassNames)
		{
			if (lvsClass.getClass().isAnnotationPresent(OKW_FN.class))
			{
				lvLsReturn.add(lvsClass);
			}
		}
		return lvLsReturn;
	}
	...
	/// \date 2015.01.28
	///
	private void AddFrameInstancesToDictionary( ArrayList<Class<?>> fpListOfOKWGuiTypes ) throws InstantiationException
	{
		...
		try
		{
			...
			for (Class<?> lvOKWGuiClass : fpListOfOKWGuiTypes)
			{
				...
				// auslesen wird.
				String lvsFunktionalname = this.GetFunktionlanameFromObjekt(lvTypeInstanceAsObject);
				...
				if (lvsFunktionalname != null && !lvsFunktionalname.isEmpty())
				{
					...
					Log.ResOpenList("\n Window Frame: >>" + lvsKey + "<<");
					Log.LogPrint("     Technisches Objekt: >>" + lvTypeInstanceAsObject.getClass().getName() + "<<");

					__FrameObjectDictionary.put(lvsFunktionalname, lvTypeInstanceAsObject);

					Log.LogPrint(this.LM.GetMessage("CreateInstanceByObjectName", "InstanceWasCreated",
		...
		try
		{
			...
			for (Field lvField : lvFields)
			{
				if (lvField.isAnnotationPresent(OKW_FN.class))
				{
					...
					Log.LogPrint("\n   Child: >>" + lvsKey + "<<");
					Log.LogPrint("Technisches Objekt: >>" + lvField.getClass().getName() + "<<");

					__FrameObjectDictionary.put(lvsKey, lvField);

					// Dann im Objket weiter Scannen...
	...
	/// \date 2014.10.10
	///
	private Object CreateInstanceByType( Class<?> fpParentType ) throws InstantiationException
	{
		Object lvo_Obj = null;
		Boolean bOK = false;

		Log.LogFunctionStartDebug("FrameObjectDictionary.CreateInstanceByObjectName", "fpParentClass",
		...
		try
		{
			...
			Log.LogPrintDebug(this.LM.GetMessage("CreateInstanceByObjectName", "InstanceWasCreated",
					lvo_Obj.getClass().getName()));
			bOK = true;
		}
		catch (IllegalAccessException e)
		{
			...
			throw new OKWFrameObjectParentNotFoundException(
					this.LM.GetMessage("CreateInstanceByObjectName", "InstanceWasCreated", fpParentType.getName()));
		}
		finally
		{
			if (bOK)
	...
	/// \param fps_ChildName Fachlicher Name des Kind-Objektes im Fenster-Objekt
	/// fps_ParentName.
	///
	/// \~english
	/// \brief
	...
	/// Obejkt angelegt?
	private Object __CreateInstanceByObjectName( String fps_ParentObject, String fps_ChildObject )
	{
		Log.LogFunctionStartDebug("FrameObjectDictionary.CreateInstanceByObjectName", "fps_ParentObject",
				fps_ParentObject, "fps_ChildObject", fps_ChildObject);
		...
		try
		{
			...
			if (myParentObject != null)
			{
				...
				myChildObject = this.GetChildObjectByName(myParentObject, fps_ChildObject);
			}
			else
			{
	...
	/// \author Zoltan Hrabovszki
	/// \date 2014-10-10
	private Object GetChildObjectByName( Object fpo_ParentObject, String fps_ChildName )
	{
		Object lvo_Return = null;

		Log.LogFunctionStartDebug("FrameObjectDictionary.GetChildObjectByName", "fpo_ParentObject",
				fpo_ParentObject.toString(), "fps_ChildName", fps_ChildName);
		...
		try
		{
			...
			Log.LogPrintDebug(this.LM.GetMessage("GetChildObjectByName", "M1", fps_ChildName,
					fpo_ParentObject.getClass().getName()));

			// Ja, das Objekt existiert.
			Log.LogPrintDebug(this.LM.GetMessage("GetChildObjectByName", "M2"));
		...
		catch (NoSuchFieldException e)
		{
			...
			Log.LogPrintDebug(this.LM.GetMessage("GetChildObjectByName", "M3"));
			throw new OKWFrameObjectChildNotFoundException(this.LM.GetMessage("GetChildObjectByName",
					"ChildIsNotDefined_Exception", fps_ChildName, fpo_ParentObject.getClass().getName()));
		}
		finally
		{
			Log.LogFunctionEndDebug();
