...
public class FrameObjectDictionary_Sngltn
{
	...
	/// \date 2014.10.10
	///
	public void Init() throws ClassNotFoundException, InstantiationException, JAXBException,
	...
	/// \date 2015.01.28
	///
	private void AddFrameInstancesToDictionary( ArrayList<Class<?>> fpListOfOKWGuiTypes )
		...
		try
		{
			...
			for (Class<?> lvOKWGuiClass : fpListOfOKWGuiTypes)
			{
				...
				if (lvsFunktionalname != null && !lvsFunktionalname.isEmpty())
				{
					...
					myFrameObjectDictionary.put(lvsFunktionalname, lvTypeInstanceAsObject);

					Log.LogPrint(LM.GetMessage("CreateInstanceByObjectName", "InstanceWasCreated",
	...
	private Object CreateInstanceByType( Class<?> fpParentType ) throws InstantiationException, XPathExpressionException
	{
		...
		try
		{
			...
			Log.LogPrintDebug(
					LM.GetMessage("CreateInstanceByObjectName", "InstanceWasCreated", lvo_Obj.getClass().getName()));
			bOK = true;
		}
		catch (IllegalAccessException e)
		{
			...
			throw new OKWFrameObjectParentNotFoundException(
					LM.GetMessage("CreateInstanceByObjectName", "InstanceWasCreated", fpParentType.getName()));
		}
		finally
		{
			if (bOK)
	...
	/// Obejkt angelegt?
	private Object CreateInstanceByObjectName( String fps_ParentObject, String fps_ChildObject )
			throws XPathExpressionException
	{
	...
	private Object GetChildObjectByName( Object fpo_ParentObject, String fps_ChildName ) throws XPathExpressionException
	{
		...
		try
		{
			...
			Log.LogPrintDebug(
					LM.GetMessage("GetChildObjectByName", "M1", fps_ChildName, fpo_ParentObject.getClass().getName()));
		...
		catch (NoSuchFieldException e)
		{
			...
			Log.LogPrintDebug(LM.GetMessage("GetChildObjectByName", "M3"));
			throw new OKWFrameObjectChildNotFoundException(LM.GetMessage("GetChildObjectByName",
